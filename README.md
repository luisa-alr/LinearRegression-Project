...
+ To see the answer to the questions they will printed out to the terminal, but are also included here and in the pdf
...
1) - We want to get two matrices and they need to be the same size to perform any operations.
x = (x1, x2, ... , xd)
...
3) - Mean Squared Error (MSE) finds the value who minimizes the errors.
E(w) = 1/n ||Xw-y||^2
take the difference of the predicted and actual values and square the values, then take the average.

4) - The derivative of E(w) = 0 gives us the minimizing error measure. Which will allow us to choose the lambda that gives the least test set MSE.
