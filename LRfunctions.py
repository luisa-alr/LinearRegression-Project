...
#L2 function for a single lambda
def L2_1_lambda(x_mtx, y_mtx, l):
    # w = (xTx + lambdaI)^-1 xTy
    x_mtx_T = np.transpose(x_mtx)  # transpose x matrix
    xTx = np.dot(x_mtx_T, x_mtx)  # multiply matrices
    I = np.identity(len(xTx))  # create identity matrix
    lambdaI = np.multiply(l, I)  # lambda * identity matrix
    parenthesis = xTx + lambdaI  # sum
    inverse = np.linalg.inv(parenthesis)  # take inverse (^-1)
    xTy = np.dot(x_mtx_T, y_mtx)  # multiply matrices
    w = np.dot(inverse, xTy)  # find w
...
def plot_MSE(MSE_train_ds, MSE_test_ds, ds1, size1, size2, l):
    MSE_train_plot = plt.plot(
        MSE_train_ds, label=f"MSE Dataset {ds1} - Train", color="darkorange"
    )
    MSE_test_plot = plt.plot(
        MSE_test_ds, label=f"MSE Dataset {ds1} - Test", color="dodgerblue"
    )
    plt.title(f"Train-{size1} vs Test-{size2}\n lambda {l}")
    plt.xlabel("lambda")
    plt.ylabel("MSE")
    # to adjust y-size of the plots, but not extremelly necessary since the plot will be automatically generated without "cutting off" any data.
    #plt.ylim(0, 20) 
    plt.legend()
    plt.show()
...
def CV(K_value, y_mtx, x_mtx, start, l_range):
    ...
    bestl, min_mse = Least_Test_Set(mse_test)
...
def LC(x_train, y_train, x_test, y_test, fixed_lambda, rep, max_size, step):
    for l in fixed_lambda:
        ...
        plt.title(f"Learning Curve 1000-100 \nlambda = {l}")
